...
import { createLostFrame, safeValidateFrame } from './contracts.js';
...
export class MediaPipeAdapter implements IPointerFramePort {
	...
	constructor(id = 'mediapipe-1', config: Partial<AntiTheaterConfig> = {}) {
		this.id = id;
	...
	private processFrame(): void {
		...
		const results: HandLandmarkerResult = this.handLandmarker.detectForVideo(this.videoElement, ts);
	...
	private mapGesture(gestures?: HandGesture[]): GestureLabel {
		...
		return validGestures.includes(name as GestureLabel) ? (name as GestureLabel) : 'None';
	}
	...
	private addViolation(violation: TheaterViolation): void {
		...
		const recentViolations = this.violations.filter(
			(v) => v.ts > latestTs - 5000 && (v.severity === 'error' || v.severity === 'quarantine'),
		);
