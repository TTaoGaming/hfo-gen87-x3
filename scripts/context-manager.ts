...
function getGitChangedFiles(since: string = 'HEAD~10'): string[] {
  try {
...
function getRecentBlackboardSignals(count: number = 5): BlackboardSignal[] {
  const bbPath = path.join(process.cwd(), 'obsidianblackboard.jsonl');
  if (!fs.existsSync(bbPath)) return [];
  
  const lines = fs.readFileSync(bbPath, 'utf-8').split('\n').filter(l => l.trim());
...
function generateManifest(policy: ContextPolicy = DEFAULT_POLICY): ContextManifest {
  ...
  if (criticalTokens > policy.tiers.critical) {
    recommendations.push(`CRITICAL tier over budget (${criticalTokens}/${policy.tiers.critical}). Summarize large files.`);
  }
  
  const staleActiveFiles = active.filter(f => f.isStale);
  if (staleActiveFiles.length > 0) {
    recommendations.push(`${staleActiveFiles.length} files in ACTIVE tier are stale. Consider archiving.`);
  }
  
  if (usedTokens > policy.maxTotalTokens) {
    recommendations.push(`Total tokens (${usedTokens}) exceeds budget (${policy.maxTotalTokens}). Run compact.`);
  }
...
function generateColdStartPayload(manifest: ContextManifest): string {
  ...
  const signalLine = signals.length > 0 
    ? signals.map(s => `[${s.hive}:${s.port}]`).join(' → ')
    : 'none';
  
  const lastMsg = signals.length > 0 
    ? signals[signals.length - 1].msg.substring(0, 50)
    : 'No signals';

  // Phase names
  const phaseNames: Record<string, string> = {
    'H': 'HUNT', 'I': 'INTERLOCK', 'V': 'VALIDATE', 'E': 'EVOLVE', 'X': 'UNKNOWN'
  };
...
switch (command) {
  case 'generate': {
    ...
    console.log(`  Critical: ${manifest.tiers.critical.files.length} files (${manifest.tiers.critical.tokens} tokens)`);
    console.log(`  Active: ${manifest.tiers.active.files.length} files (${manifest.tiers.active.tokens} tokens)`);
