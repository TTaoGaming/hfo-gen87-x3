...
const REWARD_HACK_PATTERNS = [
	...
	{ pattern: /process\.exit\s*\(\s*0\s*\)/g, msg: 'process.exit(0) in test code is a reward hack' },
];
...
const WEAK_ASSERTION_PATTERNS = [
	{
		...
		allowIf: (content: string, match: RegExpMatchArray) => {
			...
			return prevLine.includes('// truthy check intentional') || prevLine.includes('/* truthy */');
		},
	},
	{
...
function detectRewardHack(filePath: string, content: string): Violation[] {
	...
	// Skip health constraint test to avoid flagging pattern definitions
	if (filePath.includes('codebase-health.constraint.test.ts')) return [];
...
function detectWeakAssertions(filePath: string, content: string): Violation[] {
	...
	for (const { pattern, msg, allowIf } of WEAK_ASSERTION_PATTERNS) {
		...
		while (true) {
			...
			if (allowIf?.(content, match)) continue;
	...
	for (const pattern of skipPatterns) {
		...
		while (true) {
			...
			const hasReason =
				...
				prevLines.includes('@reason') ||
				currentLine.includes('TODO') ||
				currentLine.includes('Phase') ||
				currentLine.includes('@reason');
			...
			if (!hasReason) {
				...
				violations.push({
					...
					type: 'SKIP_ABUSE',
					message:
						'Skipped test without TODO/Phase reason. Either implement or document why skipped.',
					severity: 'warning',
				});
			}
		}
	...
	function walk(currentDir: string) {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				if (
					!['node_modules', 'dist', 'coverage', '.git', 'playwright-report'].includes(entry.name)
				) {
					walk(fullPath);
				}
...
function main() {
	...
	const targetDir = args[0] || './hot/bronze/src';
	...
	console.log(
		`Mode: ${strictMode ? 'STRICT (errors + warnings fail)' : 'NORMAL (only errors fail)'}\n`,
	);

	const result = analyzeDirectory(targetDir);

	// Also scan demos if we're scanning the default src
	if (targetDir === './hot/bronze/src') {
		const demoResult = analyzeDirectory('./demos');
		result.violations.push(...demoResult.violations);
		result.filesAnalyzed += demoResult.filesAnalyzed;
		result.errorCount += demoResult.errorCount;
		result.warningCount += demoResult.warningCount;
	}
