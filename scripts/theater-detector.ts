...
const EXEMPLAR_REGISTRY = {
  '1euro': {
    ...
    goodPatterns: [
      /import.*from\s*['"]1eurofilter['"]/i,
      /import.*OneEuroFilter.*from/i,
    ],
  },
    ...
    for (const pattern of config.handRolledPatterns) {
      ...
      for (const match of matches) {
        ...
        violations.push({
          ...
          message: hasGoodImport 
            ? `Hand-rolled ${config.name} detected (but good import exists - verify usage)`
            : `Hand-rolled ${config.name} detected - USE EXEMPLAR`,
...
async function main() {
  ...
  // Scan patterns
  const scanPatterns = [
    path.join(sandboxDir, 'demos/**/*.html'),
    path.join(sandboxDir, 'src/**/*.ts'),
    path.join(sandboxDir, 'src/**/*.tsx'),
  ];
  ...
  // Get all files
  const htmlFiles = walkDir(path.join(sandboxDir, 'demos'), ['.html']);
  const tsFiles = walkDir(path.join(sandboxDir, 'src'), ['.ts', '.tsx']);
