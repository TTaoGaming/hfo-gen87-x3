...
const HIVE_PHASES: Record<number, 'H' | 'I' | 'V' | 'E'> = {
	0: 'H',
	7: 'H', // Hunt: Lidless + Spider
	1: 'I',
	6: 'I', // Interlock: Weaver + Kraken
	2: 'V',
	5: 'V', // Validate: Magus + Pyre
	3: 'E',
	4: 'E', // Evolve: Storm + Regnant
};
...
const consoleInterval = setInterval(
	() => {
		...
		const nextHeartbeat = new Date(
			Date.now() + HEARTBEAT_INTERVAL_MS - (Date.now() % HEARTBEAT_INTERVAL_MS),
		);
		console.log(
			`[${now}] 🌪️ Spore Storm alive | Heartbeats: ${heartbeatCount} | Next: ~${Math.round((nextHeartbeat.getTime() - Date.now()) / 1000 / 60)}min`,
		);
	},
	5 * 60 * 1000,
);
