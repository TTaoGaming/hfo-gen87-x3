...
function processFSM(frame) {
  ...
  switch (fsmState.current) {
    ...
    case 'ARMED':
      ...
      } else if ((label === 'Thumb_Up' || label === 'Thumb_Down') && confidence >= FSM_CONFIG.minConfidence) {
...
function updateStateUI(state) {
  ...
  document.getElementById('ctxBaseline').textContent = 
    fsmState.baselineStableAt ? `${fsmState.baselineStableAt.toFixed(0)}ms` : 'null';
  document.getElementById('ctxArmedFrom').textContent = String(fsmState.armedFromBaseline);
  document.getElementById('ctxPosition').textContent = 
    fsmState.lastPosition ? `${fsmState.lastPosition.x.toFixed(2)}, ${fsmState.lastPosition.y.toFixed(2)}` : 'null';
}
...
function updateNoneUI(prediction) {
  const pct = Math.min(100, (prediction.msInNone / DEFAULT_GESTURE_TRANSITION_CONFIG.longNoneMs) * 100);
...
function onHandResults(results) {
  ...
  if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
    ...
    if (action.action !== 'none' && action.action !== 'move') {
      log('action', `W3C: ${action.action}${action.button !== undefined ? `(btn=${action.button})` : ''}`);
    }
    
    if (gateResult.changed) {
      log('gate', `Palm gate: ${gateResult.isFacing ? 'ARMED' : 'DISARMED'} (${palmAngle.toFixed(0)}°)`);
    }
    ...
    const prediction = updateGestureTransitionPredictor('None', ts, gestureState, DEFAULT_GESTURE_TRANSITION_CONFIG);
    gestureState = prediction.state;
