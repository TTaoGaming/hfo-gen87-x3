/**
 * HFO Port Factory (Silver Exemplar)
 *
 ...
 */
import { DoubleExponentialPredictor } from '../../bronze/src/adapters/double-exponential-predictor.adapter.js';
import { GoldenLayoutShellAdapter } from '../../bronze/src/adapters/golden-layout-shell.adapter.js';
import { MediaPipeAdapter } from '../../bronze/src/adapters/mediapipe.adapter.js';
...
import { NullOverlayAdapter } from '../../bronze/src/adapters/quarantine/stub-overlay.adapter.js';
...
class RapierSmootherAdapter implements SmootherPort {
	...
	smooth(
		frame: import('../../bronze/src/contracts/schemas.js').SensorFrame,
	): import('../../bronze/src/contracts/schemas.js').SmoothedFrame {
 ...
 * In browser, we'd use the real DOMAdapter from pointer-event.adapter.ts directly
 */
class VirtualDOMAdapter {
	...
	inject(_event: import('../../bronze/src/contracts/schemas.js').PointerEventOut): boolean {
	...
	createSmoother(): SmootherPort {
		...
		switch (smoother.type) {
			case '1euro': {
				...
				const config: import(
					'../../bronze/src/adapters/one-euro-exemplar.adapter.js',
				).OneEuroConfig = {};
			...
			case 'rapier-smooth': {
				...
				const config: Partial<
					import('../../bronze/src/adapters/rapier-physics.adapter.js').RapierConfig
				> = {
					mode: 'smoothed',
				};
			...
			case 'desp': {
				...
				// @source https://cs.brown.edu/people/jlaviola/pubs/kfvsexp_final_laviola.pdf
				const config: Partial<
					import('../../bronze/src/adapters/double-exponential-predictor.adapter.js').DESPConfig
				> = {};
