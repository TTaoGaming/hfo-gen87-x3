...
} from '../../bronze/src/adapters/golden-layout-shell.adapter.js';
import {
	HFOPortFactory,
	type ShellConfig,
	type SmootherConfig,
 ...
 */
export class TileComposer implements ComposerPort {
		...
		if (this.shell instanceof GoldenLayoutShellAdapter) {
			for (const [type, factory] of this.componentFactories) {
				(this.shell as GoldenLayoutShellAdapter).registerComponent(
					type as import('../../bronze/src/contracts/schemas.js').TileType,
					factory,
				);
			}
		}
