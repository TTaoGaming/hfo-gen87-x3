...
import type { IPointerFSMPort, FSMSnapshot, FSMState as FSMStateType } from './contracts.js';
...
type FSMEvent = { type: 'FRAME'; frame: PointerFrame } | { type: 'CANCEL' };
				...
				FRAME: [
					...
					// Must check BEFORE timeout/depleted to allow recovery
					{
						guard: ({ context, event }) => {
							if (event.type !== 'FRAME') return false;
							return (
								event.frame.trackingOk && isPalmFacing(event.frame.palmAngle, true, context.config)
							);
						},
						target: 'TRACKING',
...
/** Configuration interface for FSM initialization */
export interface W3CPointerFSMConfig {
	gateConfig?: Partial<GateConfig>;
	pointerId?: number;
}

export class W3CPointerFSM implements IPointerFSMPort {
	private actor: ActorRefFrom<typeof pointerFSM>;
	private previousState: PointerFSMState = 'IDLE';
	private config: GateConfig;

	constructor(config: Partial<GateConfig> = {}, pointerId = 1) {
 ...
 */
	...
	/**
	 * Get full FSM snapshot (for IPointerFSMPort interface)
	 */
	getSnapshot(): FSMSnapshot {
		const ctx = this.actor.getSnapshot().context;
		const state = this.getState();
		return {
			state: state as FSMStateType,
			position: { ...ctx.position },
			velocity: { ...ctx.velocity },
			wasFacing: ctx.wasFacing,
			pointerId: ctx.pointerId,
			palmStableAt: ctx.palmStableAt,
			coastStartAt: ctx.coastStartAt,
		};
	}
