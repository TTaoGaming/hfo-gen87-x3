...
describe('Contract: PointerFrameSchema', () => {
  describe('POSITIVE: Valid frames', () => {
    ...
    it('should accept all valid gesture labels', () => {
      const gestures = ['None', 'Open_Palm', 'Pointing_Up', 'Victory', 'Thumb_Up', 'Thumb_Down', 'Closed_Fist'];
      for (const gesture of gestures) {
        const frame = createDefaultFrame({ trackingOk: true, gesture: gesture as any, confidence: 0.9 });
        expect(() => validateFrame(frame)).not.toThrow();
      }
    });
    ...
    it('should accept boundary positions', () => {
      ...
      expect(() => validateFrame(createDefaultFrame({ position: { x: 0.5, y: 0.5 } }))).not.toThrow();
    });
  });
...
describe('Contract: W3CPointerActionSchema', () => {
  ...
  describe('NEGATIVE: Invalid actions', () => {
    ...
    it('should reject negative button numbers', () => {
      expect(() => validateAction({ type: 'down', x: 0.5, y: 0.5, pointerId: 1, button: -1 })).toThrow();
    });
  });
});
...
describe('Type Guards', () => {
  describe('isTrackingAction', () => {
    it('should return true for tracking actions', () => {
      ...
      expect(isTrackingAction({ type: 'down', x: 0.5, y: 0.5, pointerId: 1, button: 0 })).toBe(true);
