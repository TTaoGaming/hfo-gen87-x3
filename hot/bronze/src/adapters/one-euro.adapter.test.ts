...
describe('OneEuroAdapter', () => {
	...
	describe('smooth() - Basic Operation', () => {
		...
		it('returns position close to input for first frame', () => {
			const result = adapter.smooth(createFrame({ 
				ts: 0,
	...
	describe('smooth() - Tracking Lost Handling', () => {
		...
		it('resets filters when tracking lost', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 48, indexTip: { x: 0.8, y: 0.8, z: 0, visibility: 1 } }));
	...
	describe('smooth() - Velocity Calculation', () => {
		...
		it('calculates positive x velocity for rightward movement', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1000, indexTip: { x: 1.0, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('calculates negative x velocity for leftward movement', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1000, indexTip: { x: 0.0, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('calculates positive y velocity for downward movement', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1000, indexTip: { x: 0.5, y: 1.0, z: 0, visibility: 1 } }));
			expect(result.velocity!.y).toBeGreaterThan(0);
		});

		it('returns near-zero velocity for stationary input', () => {
			adapter.smooth(createFrame({ ts: 0, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
			const result = adapter.smooth(createFrame({ ts: 1000, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('velocity uses division by dt (not multiplication)', () => {
			...
			adapter.smooth(createFrame({ ts: 0, indexTip: { x: 0.0, y: 0.5, z: 0, visibility: 1 } }));
			const result = adapter.smooth(createFrame({ ts: 500, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
	...
	describe('smooth() - Prediction Calculation', () => {
		...
		it('prediction is clamped to [0, 1] range', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 100, indexTip: { x: 1.0, y: 0.5, z: 0, visibility: 1 } }));
	...
	describe('smooth() - Filter Smoothing Behavior', () => {
		it('smooths noisy input', () => {
			const frames = [
				createFrame({ ts: 0, indexTip: { x: 0.50, y: 0.50, z: 0, visibility: 1 } }),
		...
		it('handles NaN from filter by returning input value', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.6, y: 0.6, z: 0, visibility: 1 } }));
			// Should not be NaN
	...
	describe('reset()', () => {
		it('clears filter state', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 32, indexTip: { x: 0.9, y: 0.9, z: 0, visibility: 1 } }));
	...
	describe('setParams()', () => {
		...
		it('updates beta parameter', () => {
			...
			adapter.smooth(createFrame({ ts: 0, indexTip: { x: 0.0, y: 0.5, z: 0, visibility: 1 } }));
			const result = adapter.smooth(createFrame({ ts: 16, indexTip: { x: 1.0, y: 0.5, z: 0, visibility: 1 } }));
...
describe('MUTATION KILLER: Filter internals', () => {
	...
	describe('Derivative calculation edge cases', () => {
		it('keeps previous derivative when dt is too small (< 0.1ms)', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1000.05, indexTip: { x: 0.51, y: 0.5, z: 0, visibility: 1 } }));
	...
	describe('Filter NaN handling', () => {
		it('returns input value when filter returns NaN', () => {
			...
			adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
			const result = adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.7, y: 0.7, z: 0, visibility: 1 } }));
	...
	describe('Prediction calculation correctness', () => {
		it('prediction uses predictionMs parameter', () => {
			...
			longPredictionAdapter.smooth(createFrame({ ts: 0, indexTip: { x: 0.0, y: 0.5, z: 0, visibility: 1 } }));
			const result = longPredictionAdapter.smooth(createFrame({ ts: 1000, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('prediction clamped to minimum 0', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.05, y: 0.5, z: 0, visibility: 1 } }));
			expect(result.prediction!.x).toBeGreaterThanOrEqual(0);
		});
		...
		it('prediction clamped to maximum 1', () => {
			...
			adapter.smooth(createFrame({ ts: 0, indexTip: { x: 0.9, y: 0.5, z: 0, visibility: 1 } }));
			const result = adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.95, y: 0.5, z: 0, visibility: 1 } }));
			expect(result.prediction!.x).toBeLessThanOrEqual(1);
		});
	});
	...
	describe('Math.max/Math.min boundary tests', () => {
		it('position x clamp uses Math.max(0, ...)', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1, indexTip: { x: -0.5, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('position x clamp uses Math.min(1, ...)', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1, indexTip: { x: 1.5, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('position y clamp uses Math.min(1, ...)', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1, indexTip: { x: 0.5, y: 1.5, z: 0, visibility: 1 } }));
	...
	describe('MUTATION KILLER: Arithmetic operators', () => {
		...
		it('derivative calculation uses subtraction for position delta', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 1000, indexTip: { x: 0.8, y: 0.5, z: 0, visibility: 1 } }));
	...
	describe('MUTATION KILLER: Conditional expressions', () => {
		it('trackingOk OR indexTip null triggers no-tracking path', () => {
			...
			const noTracking = adapter.smooth(createFrame({ trackingOk: false, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('dt > 0.0001 required for derivative calculation', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 2000.05, indexTip: { x: 0.51, y: 0.5, z: 0, visibility: 1 } }));
		...
		it('lastValue undefined check on first frame', () => {
			...
			const result = adapter.smooth(createFrame({ ts: 0, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
			expect(result.velocity!.x).toBe(0);
			expect(result.velocity!.y).toBe(0);
		});
		...
		it('Number.isNaN check returns input on NaN', () => {
			...
			adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.5, y: 0.5, z: 0, visibility: 1 } }));
			const result = adapter.smooth(createFrame({ ts: 100, indexTip: { x: 0.8, y: 0.8, z: 0, visibility: 1 } }));
