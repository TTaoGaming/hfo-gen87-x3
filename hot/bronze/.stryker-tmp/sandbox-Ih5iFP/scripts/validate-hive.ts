...
function printReport(
	...
	console.log(
		`  Current Phase: ${currentPhase ? `${currentPhase} - ${PHASE_NAMES[currentPhase]}` : 'None (start with H)'}`,
	);
	...
	for (const [phase, count] of Object.entries(validation.stats.byPhase)) {
		...
		console.log(
			`  ${phase} (${PHASE_NAMES[phase as HIVEPhase].padEnd(20)}): ${count.toString().padStart(3)} ${bar}`,
		);
	}
