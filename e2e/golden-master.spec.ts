...
const SHOWCASES = [
	...
	},
	{
		...
		path: '/showcase-goldenlayout.html',
		...
		test.describe(showcase.name, () => {
			...
			if (showcase.goldenVideo) {
				test(`validates against golden landmarks (${showcase.goldenVideo})`, async ({ page }) => {
					...
					// Load landmarks data
					const landmarksData = fs
						.readFileSync(landmarksPath, 'utf-8')
						.split('\n')
