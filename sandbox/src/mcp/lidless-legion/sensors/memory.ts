...
export class MemorySensor implements SensorAdapter {
  ...
  async sense(query: string, limit: number, options?: Record<string, unknown>): Promise<SenseResult[]> {
    try {
      const db = await this.getConnection() as { all: (sql: string, cb: (err: Error | null, rows: unknown[]) => void) => void };
      ...
      return new Promise((resolve, reject) => {
        ...
        (db as { run: (sql: string, cb?: (err: Error | null) => void) => void }).run('LOAD fts', (err) => {
          if (err) {
