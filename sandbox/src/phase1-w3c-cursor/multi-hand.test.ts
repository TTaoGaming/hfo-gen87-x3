...
interface HandIdTrackerPort {
  ...
  updateIds(hands: Array<{ handedness: 'Left' | 'Right'; landmarks: unknown[] }>): TrackedHandFrame[];
...
try { MultiHandManager = require('./multi-hand-manager.js').MultiHandManager; }
catch { MultiHandManager = class { constructor() { throw new Error('MultiHandManager not implemented'); } }; }

try { HandIdTracker = require('./hand-id-tracker.js').HandIdTracker; }
catch { HandIdTracker = class { constructor() { throw new Error('HandIdTracker not implemented'); } }; }

try { PerformanceMonitor = require('./performance-monitor.js').PerformanceMonitor; }
catch { PerformanceMonitor = class { constructor() { throw new Error('PerformanceMonitor not implemented'); } }; }

try { DegradationStrategy = require('./degradation-strategy.js').DegradationStrategy; }
catch { DegradationStrategy = class { constructor() { throw new Error('DegradationStrategy not implemented'); } }; }
...
describe('PHASE 1.5 - MultiHandManager', () => {
  ...
  describe('Hand persistence across frames', () => {
    it('should maintain stable handId across frames', () => {
      ...
      manager.processFrame(createMultiHandFrame([
        { handedness: 'Right', x: 0.52, y: 0.48 }
      ]));
...
describe('PHASE 1.5 - HandIdTracker', () => {
  ...
  describe('ID assignment', () => {
    it('should assign stable IDs based on handedness', () => {
      ...
      const hands = [
        { handedness: 'Right' as const, landmarks: createMockLandmarks(0.5, 0.5) }
      ];
    ...
    it('should maintain ID when hand moves', () => {
      ...
      // Frame 2 - moved
      const hands2 = [
        { handedness: 'Right' as const, landmarks: createMockLandmarks(0.52, 0.48) }
      ];
...
function createMultiHandFrame(hands: Array<{
  handedness: 'Left' | 'Right';
  x: number;
  y: number;
}>): MultiHandFrame {
...
function createMockLandmarks(x: number, y: number): Array<{ x: number; y: number; z: number }> {
  return Array(21).fill(null).map(() => ({ x, y, z: 0 }));
}
